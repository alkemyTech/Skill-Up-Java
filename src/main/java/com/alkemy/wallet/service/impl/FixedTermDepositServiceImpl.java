...
public class FixedTermDepositServiceImpl implements IFixedTermDepositService {
    ...
    public FixedTermDepositResponseDto save(FixedTermDepositRequestDto requestDto, String token) {
        FixedTermDeposit fixedTermDeposit = mapper.dto2Entity(requestDto);
        ...
        if (fixedTermAccount == null)
            throw new IllegalArgumentException(String.format("The accountId: %s is not valid o does not belong to an user account",
        ...
        if ((closingDateDays >= 30) && (fixedTermAccount.getBalance() >= fixedDepositAmount)) {
            ...
            throw new IllegalArgumentException(
                    String.format("Closing Date is less than 30 days: %s  or account has not enough money: %s",
                            closingDateDays, fixedDepositAmount));
        }
